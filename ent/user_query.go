...
func (_q *UserQuery) QueryStrategies() *StrategyQuery {
...
func (_q *UserQuery) QueryPortfolios() *PortfolioQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPortfolios: _q.withPortfolios.Clone(),
...
func (_q *UserQuery) WithStrategies(opts ...func(*StrategyQuery)) *UserQuery {
	query := (&StrategyClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithPortfolios(opts ...func(*PortfolioQuery)) *UserQuery {
	query := (&PortfolioClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPortfolios = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withStrategies; query != nil {
		if err := _q.loadStrategies(ctx, query, nodes,
			func(n *User) { n.Edges.Strategies = []*Strategy{} },
			func(n *User, e *Strategy) { n.Edges.Strategies = append(n.Edges.Strategies, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPortfolios; query != nil {
		if err := _q.loadPortfolios(ctx, query, nodes,
...
func (_q *UserQuery) loadStrategies(ctx context.Context, query *StrategyQuery, nodes []*User, init func(*User), assign func(*User, *Strategy)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(strategy.FieldUserID)
	}
	query.Where(predicate.Strategy(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.StrategiesColumn), fks...))
	}))
...
func (_q *UserQuery) loadPortfolios(ctx context.Context, query *PortfolioQuery, nodes []*User, init func(*User), assign func(*User, *Portfolio)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
